    ...
    for epoch in range(current_epoch, cfg['EPOCHS']):
        ...
        for num_batch, batch in enumerate(train_dl):
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                losses_dict = model(run_params, batch, device)
...
def create_run_folder(SEED):
    run_folder_path = os.path.join(path_runs, exp_name, f"run_{RUN}", str(SEED))
...
def main(SEED):
    ...
    c_idx_brown_test = concordance_index_censored(event_labels, time_labels, risk_scores)[0]
    print(c_idx_brown_test)
    ...
    c_idx_penn = concordance_index_censored(event_labels, time_labels, risk_scores)[0]
    print(c_idx_penn)
    return c_idx_brown_test, c_idx_penn, best_epoch 

if __name__ == "__main__":
    exp_name = 'base_SP_model_seed45_no_nofix'
    out_folder = os.path.join(path_runs, exp_name, f"run_{RUN}")
    
    out_filename = 'c_idx_results.xlsx'
    out_xlsx_path = join(out_folder, out_filename)
    if isfile(out_xlsx_path):
        df = pd.read_excel(out_xlsx_path)
        out_index_dict = {'rand_seed': df['rand_seed'].values.tolist(), 'brown_cidx':df['brown_cidx'].values.tolist(), 'penn_cidx':df['penn_cidx'].values.tolist(), 'best_epoch':df['best_epoch'].values.tolist()}
        start = out_index_dict['rand_seed'][-1]+1
    else:
        out_index_dict = {'rand_seed': [], 'brown_cidx':[], 'penn_cidx':[], 'best_epoch':[]}
        start = 0
    c_idx_brown_test, c_idx_penn, best_epoch = main(45)
    out_index_dict['rand_seed'].append(45)
    out_index_dict['best_epoch'].append(best_epoch)
    out_index_dict['brown_cidx'].append(c_idx_brown_test)
    out_index_dict['penn_cidx'].append(c_idx_penn)
    out_df = pd.DataFrame.from_dict(out_index_dict)
    out_df.to_excel(out_xlsx_path)
    # for i in range(start, 100):
    #     try:
    #         c_idx_brown_test, c_idx_penn, best_epoch = main(i)
    #     except:
    #         continue
    #     out_index_dict['rand_seed'].append(i)
    #     out_index_dict['best_epoch'].append(best_epoch)
    #     out_index_dict['brown_cidx'].append(c_idx_brown_test)
    #     out_index_dict['penn_cidx'].append(c_idx_penn)
    #     out_df = pd.DataFrame.from_dict(out_index_dict)
