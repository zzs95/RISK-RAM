...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            image_accnum = int(clin_report['Accession Number'])
...
class CustomCollator():
    ...
    def __call__(self, batch: list[dict[str]]):
        ...
        accNum_batch = torch.empty(size=(batch_size, ), dtype=int)
        ...
        for i, sample_dict in enumerate(batch):
            ...
            accNum_batch[i] = sample_dict.pop("accNum")
...
def get_datasets_as_dfs(setname='brown', random_state_i=42, text_feat='bert', do_ignore=True):
    usecols = ["days", "death", 'Accession Number']
    usecols += CLIN_KEYS
    usecols += REPORT_KEYS
